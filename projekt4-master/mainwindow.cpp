...
void MainWindow::zapisz_ruch(QKeyEvent *zdarzenie)
{
    int p=ui->predkosc->value();
    Point * struktura = new Point;

    if (Qt::Key_Return)
        struktura->czy_byl_Enter = 1;
    else struktura->czy_byl_Enter = 0;

    switch(zdarzenie->key())
    {
        case Qt::Key_Up:
            struktura->punkt0=punkt3 + QPoint(0, -p);
            break;
        case Qt::Key_Down:
            struktura->punkt0=punkt3 + QPoint(0, p);
            break;
        case Qt::Key_Left:
            struktura->punkt0=punkt3 + QPoint(-p, 0);
            break;
        case Qt::Key_Right:
            struktura->punkt0=punkt3 + QPoint(p, 0);
            break;
        default:
            struktura->punkt0=punkt3;
            break;
    }

    zapisany_ruch.push_back(*struktura);
    delete [] struktura;
}

void MainWindow::odtworz_ruch()
{
    for(int i=0; i<zapisany_ruch.size(); i++)
    {
        UstawRamie(zapisany_ruch[i].punkt0);
        if(zapisany_ruch[i].czy_byl_Enter)
            zlap();
    }
}
